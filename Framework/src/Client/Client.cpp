...
void separated_tests(const configs* const settings, secure_connection* conn) {
	...
	uint8_t result;
     for(int i=0; i < settings->bisen_queries.size(); i++)
   std::cout << settings->bisen_queries.at(i) << endl;
        //bisen_search(conn, &client, settings->bisen_queries,&result);
...
//        reset_bytes();
    ...
    if(settings->use_images) {
        ...
        if(settings->visen_nr_queries > 0) {
            // flickr
            search_flickr(conn, desc, files_search, settings->visen_nr_queries,&output);
        } else {
            // search test (inria)
